...
class WechatApi(WechatBasic):
        ...
        try:
            ...
            for reurl in recent_article_urls:
                if 'mp.weixin.qq.com' in reurl:
                    reurls.append(reurl)
            return reurls
        except WechatRequestsException as e:
    ...
    def get_sugg(self, keyword):
        ...
        Returns:
            sugg: 联想关键词列表
        Raises:
            WechatException: get_sugg keyword error 关键词不是str或者不是可以str()的类型
            WechatException: sugg refind error 返回分析错误
        """
        try:
            keyword = str(keyword) if type(keyword) != str else keyword
        except:
            raise WechatException('get_sugg keyword error')
        ...
        try:
            ...
            raise WechatException('sugg refind error')