...
class WechatBasic(WechatBase):
    ...
    def get(self, url, host='', referer='', proxy=False):
        ...
        Returns:
            text: 请求url的网页内容
        Raises:
            WechatException: 操作频繁以致出现验证码或requests请求返回码错误
        """
        headers = {
        ...
        if r.status_code == requests.codes.ok:
            ...
            if '用户您好，您的访问过于频繁，为确认本次访问为正常用户行为，需要您协助验证' in r.text:
                self.vcode_url = url
                raise WechatVcodeException('weixin.sogou.com verification code')
        else:
            raise WechatRequestsException('requests status_code error', r.status_code)
        return r.text
    ...
    def jiefeng(self, ruokuai=False):
        ...
        Args:
            ruokuai: 是否采用若快打码平台
        Raises:
            WechatVcodeException: 解封失败，可能验证码识别失败
        """
        ...
        if remsg['code'] != 0:
            raise WechatVcodeException('cannot jiefeng because ' + remsg['msg'])
